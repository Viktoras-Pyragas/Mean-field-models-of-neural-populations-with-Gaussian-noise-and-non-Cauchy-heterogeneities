...
% heterogeneity eta distributed as a Gaussian function;
% here the Gaussian distribution is generated deterministically;
%------------------------------------------------------------------------
close all;
clear;
format long;
...
sig2=0.05;% Gaussian noise amplitude (changed)
sig=sqrt(2*sig2);
del0=0.26; % % half-width (corresponds to del=d) (original)
d=del0;
...
%   s=0; % initial conditions;
  load('init_cond_micro_changed_sig2.mat', 'tet','s'); % loading initial conditions;
...
tet0p=pi-2*tsp;
px=linspace(-5,5,N); 
p=sqrt(log(2)/pi)*exp(-px.^2*log(2)); % Gaussian distribution function;
zeta=randpdf_my(p,px,[N,1]); % heterogeneity parameter zeta is distributed as Gaussian function;
% with average 0 and HWHM 1;

eta=d*zeta+muv; % the full parameter of heterogeneity;
rm=zeros(Nt,1);
...
save('micro_changed_sig2.mat','tm','rm'); % recording time series of the solution;
%-----------------------------------------------------------------------------------------
save('init_cond_micro_changed_sig2.mat', 'tet','s'); % recording final state of the system;
