function x=randpdf_my(p,px,dim)
% RANDPDF
...
pxi=(linspace(min(px),max(px),1000))';
pin=interp1(px,p,pxi,'linear');
% computing the cumulative distribution function for input pdf
cdfp = cumtrapz(pxi,pin);
...
%uniformDistNum=rand(dim);
  N=max(dim);
 uniformDistNum=(linspace(1/N,1-1/N,N)); % Changed; 
...
if nargout==0
    ...
    subplot(3,4,9:12)
    plot(userDistNum,'.') % mano pakeitimas
