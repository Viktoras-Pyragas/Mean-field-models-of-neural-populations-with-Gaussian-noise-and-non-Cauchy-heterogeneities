...
% Isskaitopa sinapsio lygtis
% cia naudojamas modelis su (a1,a2) kintamaisias
% palikti visi nariai 
%close all
clearvars
del=0.2; % half width at half maximum (HWHM)
J=2;
muv=1; % eta average
tau=1; % membane time
taus=1; % synaptic time
n=10; % aproksimacijos eile
sig2=0.04 ; % intensity of the Gaussian noise
%zeta=zet(m); % zeta(m) koeficientu pagaminimas
zeta=exp(1i*pi*(2*(1:n)-1)/2/n);
...
C=(n/pi)*sin(pi/(2*n));
g=C./(1+x.^(2*n));
gL=(1/pi)*1./(1+x.^2); % Caushy (Lorenco) skirstinys
